    ...
    def suggest(self, n_suggestions=1, fix_input = None):
        ...
        if self.X.shape[0] < self.rand_sample:
            ...
            try:
                ...
                model.fit(X, Xe, y)
            # y = torch.FloatTensor(self.y).clone()
            # percentile = torch.quantile(y, 0.05, dim=0)
            # min_y = torch.min(y, dim=0)[0]
            # max_y = torch.max(y, dim=0)[0]
            # min_y = min_y - (
            # percentile - min_y
            # )  # Subtract the difference between the percentile and the minimum
            # min_y -= 10e-6  # Minimal value to avoid numerical issues in the log scaling below

            # # Linear scaling
            # # prevent diving by zero
            # min_y[torch.where(min_y == max_y)] *= 1 - 10**-6
