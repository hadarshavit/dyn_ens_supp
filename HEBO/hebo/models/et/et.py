...
import torch
# from skopt.learning import ExtraTre/esRegressor
...
class ET(BaseModel):
    def __init__(self, num_cont, num_enum, num_out, **conf):
        ...
        self.et = ExtraTreesRegressor(n_estimators=self.n_estimators, random_state=0)
    ...
    def fit(self, Xc : torch.Tensor, Xe : torch.Tensor, y : torch.Tensor):
        ...
        self.et.fit(Xtr, ytr)
        mse = np.mean((self.et.predict(Xtr).reshape(-1) - ytr)**2).reshape(self.num_out)
    ...
    def predict(self, Xc : torch.Tensor, Xe : torch.Tensor):
        X     = self.xtrans(Xc, Xe)
        mean  = self.et.predict(X).reshape(-1, 1)
        preds = []
        for estimator in self.et.estimators_:
