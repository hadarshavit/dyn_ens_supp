...
class RFSKOPT(BaseModel):
    ...
    def predict(self, Xc : torch.Tensor, Xe : torch.Tensor):
        X     = self.xtrans(Xc, Xe)
        mean, var = self.rf.predict(X, return_std=True)
        mean = torch.FloatTensor(mean.reshape([-1,1]))
        var = torch.FloatTensor(var.reshape([-1,1]))
        # var = torch.pow(var, 2)
        mean[torch.isnan(mean)] = 0
        var[torch.isnan(var)] = 0
        return mean, var + self.noise
