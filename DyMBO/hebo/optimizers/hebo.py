...
def calc_percentile(t: torch.tensor, q: float):
    """
...
class HEBO(AbstractOptimizer):
    ...
    def __init__(self, space, model_name = 'gp', rand_sample = None, acq_cls = MACE, es = 'nsga2', model_config = None,
        ...
        if self.acq_cls != MACE:
            self.return_pop = True
        else:
            self.return_pop = False
    ...
    def suggest(self, n_suggestions=1, fix_input = None):
        if self.X.shape[0] < self.rand_sample:
            ...
            rec = opt.optimize(initial_suggest = best_x, fix_input = fix_input, return_pop=self.return_pop).drop_duplicates()
            rec = rec[self.check_unique(rec)]

            if not self.return_pop:
                cnt = 0
                ...
                rec = rec.iloc[:n_suggestions, :] # single objective, just take the 8 first in pop
                cnt = 0
                while rec.shape[0] < n_suggestions:
                    rand_rec = self.quasi_sample(n_suggestions - rec.shape[0], fix_input)
                    rand_rec = rand_rec[self.check_unique(rand_rec)]
