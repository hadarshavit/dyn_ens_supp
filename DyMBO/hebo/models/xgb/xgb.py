...
class GB(BaseModel):
    def __init__(self, num_cont, num_enum, num_out, **conf):
        ...
        self.gb = GradientBoostingRegressor(n_estimators=self.n_estimators, random_state=0)#GradientBoostingQuantileRegressor(base_estimator=GradientBoostingRegressor(n_estimators=self.n_estimators, loss='quantile'))
    ...
    def fit(self, Xc : torch.Tensor, Xe : torch.Tensor, y : torch.Tensor):
        ...
        self.gb.fit(Xtr, ytr)
        mse = np.mean((self.gb.predict(Xtr).reshape(-1) - ytr)**2).reshape(self.num_out)
    ...
    def predict(self, Xc : torch.Tensor, Xe : torch.Tensor):
        X     = self.xtrans(Xc, Xe)
        mean  = self.gb.predict(X).reshape(-1, 1)
        preds = []
        for estimator in self.gb.estimators_:
            preds.append(estimator.item().predict(X).reshape([-1,1]))
